...
import java.io.*;
/**
 * A class to store Video Tape data in a random access data file
 * 
 * @author Richard 
 * @version 210903
 */
public class VideoStoreRandom
{
    ...
    static final String FILENAME = "g:\\bluej\\videos_r.dat";
	...
	private void list()
	{
	  try
	  {
	    ...
	    for(int x = 0; x < numVideos; x = x + 1)
	    {
	      ...
	      output("" + x + ": " + VideoTapeFile.readRecord(theFile, posToRead).toString());
	    }
	  }
	  catch(IOException io)
	  {
	...
	private void quit() 
	{
	  ...
	  {
	    output("Some error closing data file -  " + io.getMessage());
	  }
	  finally
	  {
	    output("Bye then");
